...
class SaveResult:
    ...
    def show(self, Fscd, IoU_mean, Sek, Acc, refer_metric=np.array(0), best_metric=0, lr=0, epoch=0):
        # 修改：添加兼容性处理,支持标量和数组类型
        def _to_float(val):
            """将标量或数组转换为浮点数"""
            if isinstance(val, (int, float)):
                return float(val)
            elif hasattr(val, 'mean'):  # numpy数组或tensor
        ...
        with open(self.result_save_path, "a") as f:
            f.write(
                ('%-7s' + '%-12s' * 4) % (str(epoch),
                                          str(round(_to_float(Fscd), 6)),
                                          str(round(_to_float(IoU_mean), 6)),
                                          str(round(_to_float(Sek), 6)),
                                          str(round(_to_float(Acc), 6)),) + "\n")
